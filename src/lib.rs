...
pub fn parse_partial_merkle_tree(tree: PartialMerkleTree) -> Result<ParsedPartialMerkleTree, Error> {
...
impl PartialMerkleTreeBuilder {
	...
	pub fn parse(tree: PartialMerkleTree) -> Result<ParsedPartialMerkleTree, Error> {
		...
		match partial_merkle_tree.parse_tree() {
		    Ok(merkle_root) => Ok(ParsedPartialMerkleTree::new(merkle_root, partial_merkle_tree.all_hashes, partial_merkle_tree.all_matches)),
            Err(s) => Err(s),
        }
	}
	...
	fn parse_branch(&mut self, height: usize, pos: usize, matches_used: &mut usize, hashes_used: &mut usize) -> Result<H256, Error> {
		if *matches_used >= self.matches.len() {
			return Err(Error::ALLUSED);
		}
			...
			match self.parse_branch(height - 1, pos << 1, matches_used, hashes_used) {
                Ok(left) => {
					...
					let right = if has_right_child {
						match self.parse_branch(height - 1, (pos << 1) + 1, matches_used, hashes_used) {
                             Err(s) => return Err(s),
                             Ok(right)  => right,
                        }
					} else {
